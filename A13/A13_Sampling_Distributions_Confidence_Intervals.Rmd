...
```{solution}
Generally normal curve shaped, centered at the actual slope, spread seems to be +-0.2
```
...
```
```{solution}
b. the means are all very similar, but with sample size 10 being the farthest from the actual value and sample size 200 being closest
c. sample size 10 has the greatest standard error, and sample size 200 has the least
```
...
```{solution}
a. For every additional percent non hispanic white that a county is, the county is estimated to be 0.0855% less food insecure
b. -0.108 to -0.063 (-0.08551150 - 2 * 0.011265002 to -0.08551150 + 2 * 0.011265002)
c. not the same but very close
d. 0
e. 
```
...
```{solution}
a. wider
b. -inifinty to +inifity
c. no, you need to choose a confidence interval based on your schenario, a CI too big might include too many things
```
